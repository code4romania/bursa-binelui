<template>
    <div>
        ...
        <label :for="id" :class="[`block text-sm font-medium leading-6 text-${color}`]">
            <span v-if="label" class="text-sm font-semibold leading-6 text-gray-900">{{ label }}</span>
            <span v-else>
                <p class="text-sm font-normal text-gray-500"><slot /></p
            ></span>
            <br />
        ...
        <p class="text-xs font-normal" :class="{ 'text-red-600': remainingNumberOfCharters === 0 }">
            {{ remainingNumberOfCharters }}
        </p>
