<template>
    <div class="flex items-center gap-x-4">
        ...
        <UserCircleIcon v-else class="w-12 h-12 fill-gray-300" />
        <label
            :for="id"
            :class="[
                'relative inline-flex text-gray-700 bg-white ring-inset ring-1 ring-gray-300 items-center justify-center px-4 py-2 text-sm font-medium border border-transparent rounded-md',
            ]"
        >
            <span>{{ label }}</span>
            <input
...
<script setup>
    ...
    onMounted(() => {
        ...
        const file = new File([responseFile], responseFile.name, {
