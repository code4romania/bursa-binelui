    ...
    @case('project_info')
        <div class="flex ml-4">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-gray-700 group-hover:text-gray-900">{{$getRecord()->name}}</p>
                    <p class="text-xs font-medium text-gray-500 group-hover:text-gray-700">{{$getRecord()->organization->name}}</p>
                </div>
            </div>
        </div>
    @break
    @default
        <p> Add template in resources/views/tables/columns/resource-name-column.blade.php</p>
@endswitch
